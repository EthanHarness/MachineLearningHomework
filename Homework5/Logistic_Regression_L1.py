...
class LogisticRegressionL1:
    ...
    def trainModelOverHyperParameters(trData: List[List[float]], trLabel: List[int], \
            tsData: List[List[float]], tsLabel: List[int], lr: float, iterations: int, hypers: List[float]) -> dict[float, tuple[float, LogisticRegressionL1|None]]:
        
        hyperToAccDict: dict[float, tuple[float, LogisticRegressionL1|None]] = {x: (0, None) for x in hypers}
        for hyper in hypers: 
            md = LogisticRegressionL1(trData, trLabel, lr, iterations, hyper)
            acc = LogisticRegressionL1.computeAccuracyForHypers(tsData, tsLabel, md)
            hyperToAccDict[hyper] = (acc, md)

        return hyperToAccDict

    @staticmethod
    def computeAccuracyForHypers(tsData, tsLabel, model: LogisticRegressionL1) -> float:
        count = 0
